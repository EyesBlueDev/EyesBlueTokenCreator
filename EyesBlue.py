...
'''
This generator was developed by https://github.com/EyesBlueDev/EyesBlueDev
Give credits :)
...
class Info:
    ...
    def title():
        ...
        while True: os.system(f'title EyesBlue Generator - Generated: {Info.genned} ^| Solved Captchas: {Info.bypassed}')
...
class Menu:
    ...
    Info.Title('BlueEyes Token Generator - Main Menu')
    Info.cls()
                ...
                with Client(transport=SyncProxyTransport.from_url(f'{Menu.proxytype}://{self.GetProxy()}')) as request:
                    r = request.post('https://discord.com/api/v9/auth/register', headers=self.headers,
                                     json={'username': f'{Menu.username} | {secrets.token_urlsafe(4)}',
                                           'email': secrets.token_hex(6) + '@gmail.com', 'password': 'BlueEyes1001',
